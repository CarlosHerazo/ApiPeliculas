...
const MovieBanner = ({ Peliculas }) => {
    ...
    const handleCloseModal = () => {
        setOpenModal(false);
        setTrailerKey('');
        setDisableChange(false); // Habilita el cambio de películas nuevamente cuando se cierra el modal
    };
    ...
    const AbrirTrailer = async (idMovie) => {
        ...
        try {
            ...
            if (trailerKey != 0) {
                ...
                setOpenModal(true);
                setDisableChange(true); // Deshabilita el cambio de películas mientras el modal está abierto
            } else {
                // Si no se encuentra el trailer oficial, mostrar una alerta
                return <Alert severity="warning">Esta película no tiene trailer oficial.</Alert>;
            }
        } catch (error) {
            // Manejo de errores
